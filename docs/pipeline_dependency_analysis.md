...
## 三十三、UI 交互与体验优化建议（wxWidgets/wxAuiManager 架构）

针对 CameraToolKit 涉及的复杂 ISP 链路控制、多源参数加载以及高位深图像展示，以下是针对 wxWidgets/wxAuiManager 架构下的 UI 交互与体验优化建议。

### 33.1 布局与信息层级优化

由于 ISP Pipeline 节点较多（N0~N8），如果全部堆砌在一个面板中会非常混乱。

#### 33.1.1 节点式流水线编辑器 (Pipeline Editor)

**建议**：仿照 Nuke 或 Blender 的 Shader Node 界面，建立一个左侧"节点清单/属性页" + 中间"Pipeline 可视化连线"的布局。

```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│                           Pipeline Editor 界面布局                                    │
├─────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                     │
│  ┌────────────────┐  ┌──────────────────────────────────────┐  ┌────────────────┐ │
│  │  节点清单       │  │       Pipeline 可视化连线           │  │  参数属性页     │ │
│  ├────────────────┤  ├──────────────────────────────────────┤  ├────────────────┤ │
│  │                │  │                                      │  │                │ │
│  │  [✓] N0 OB     │  │   ┌─────┐    ┌─────┐    ┌─────┐     │  │  当前选中：     │ │
│  │  [✓] N1 DPC    │  │   │ N0  │───▶│ N1  │───▶│ N2  │     │  │  N5 WB         │ │
│  │  [ ] N2 LSC    │  │   │ OB  │    │ DPC │    │ LSC │     │  │                │ │
│  │  [✓] N3 Denoise│  │   └─────┘    └─────┘    └─────┘     │  │  R Gain:       │ │
│  │  [✓] N4 Demosaic│  │      │          │          │       │  │  [1.25]        │ │
│  │  [✓] N5 WB     │  │      ▼          ▼          ▼       │  │                │ │
...
│  │  标定数据源：   │  │      ▼          ▼          ▼       │  │                │ │
│  │  [✓] SCM       │  │   ┌─────┐    ┌─────┐    ┌─────┐     │  │  来源：        │ │
│  │  [ ] Color Char│  │   │ N6  │───▶│ N7  │───▶│ N8  │     │  │  [Quick Calib] │ │
│  │  [✓] Quick Calib│  │   │ CCM │    │ TM  │    │ GM  │     │  │                │ │
│  │                │  │   └─────┘    └─────┘    └─────┘     │  │  ──────────── │ │
│  │                │  │                                      │  │  [重置参数]    │ │
│  │                │  │   数据流可视化：                      │  │  [应用更改]    │ │
│  │                │  │   绿色连线 = 线性空间                 │  │                │ │
...
- 用户可以通过连线直观地看到 SCM 数据流向 N0 和 N1
- 点击任意节点，右侧属性页自动切换至对应的参数配置（如 WB Gains 或 CCM Matrix）
- 连线颜色区分线性/非线性空间：绿色 = 线性（N0-N6），红色 = 非线性（N7-N8）
- 虚线表示禁用节点，数据绕过该节点直接传递

#### 33.1.2 状态驱动的 Dashboard

**建议**：在顶部或侧边栏增加一个"Pipeline 状态看板"。

**效果**：用灯光图标（🟢/🔴）实时显示当前 Pipeline 中哪些节点被开启、哪些节点数据丢失。
...
│  │  │ 🟢 │ 🟢 │ ⚪ │ 🟢 │ 🟢 │ 🟢 │ 🟢 │ 🟢 │ 🟢 │                              │ │
│  │  │ OB │DPC │LSC │ DN │ DM │ WB │CCM │ TM │ GM │                              │ │
│  │  └────┴────┴────┴────┴────┴────┴────┴────┴────┘                              │ │
│  │                                                                               │ │
│  │  标定数据源状态：                                                              │ │
│  │  ┌─────────────┬─────────────┬─────────────┐                                  │ │
│  │  │    SCM      │  Color Char │  Quick Calib│                                  │ │
│  │  │    🟢       │    🔴       │    🟢       │                                  │ │
│  │  │  已加载     │   未加载    │  已加载(当前)│                                  │ │
│  │  └─────────────┴─────────────┴─────────────┘                                  │ │
│  │                                                                               │ │
...
│  │  🔴 = 数据缺失/未加载（节点参数不可编辑）                                        │ │
│  │  ⚪ = 已禁用（数据绕过该节点）                                                  │ │
│  │  🟡 = 参数被手动覆盖（Manual-Override）                                        │ │
│  │                                                                               │ │
│  │  当前警告：                                                                    │ │
│  │  ⚠️ N6 (CCM) 参数来自 Quick Calib，但 Color Char 数据未加载                    │ │
│  │  ⚠️ N2 (LSC) 已禁用，镜头阴影校正未生效                                         │ │
│  │                                                                               │ │
│  └───────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                     │
└─────────────────────────────────────────────────────────────────────────────────────┘
...
| 图标 | 状态 | 说明 | UI 行为 |
|------|------|------|----------|
| 🟢 | 已启用 | 节点正常运行，参数已加载 | 参数可编辑 |
| 🔴 | 数据缺失 | 标定数据未加载，节点参数不可编辑 | 参数显示为灰色，提示用户加载数据 |
| ⚪ | 已禁用 | 用户手动禁用该节点 | 数据绕过该节点，连线显示为虚线 |
| 🟡 | 手动覆盖 | 用户手动修改了标定参数 | 显示 `[Manual-Override]` 标签，高亮色显示 |

### 33.2 调试与观测体验（增强"透明度")

ISP 是一个典型的"黑盒"流程，UI 必须让数据流透明化。

#### 33.2.1 对比查看器 (Split-view Comparison)

**建议**：在图像展示区域增加"双窗口对照"或"拖拽滑块（Swipe）"对比功能。

**场景**：左侧显示 Raw 原始数据（或 OB 校正后），右侧实时显示 N8 Gamma 后的效果。
...
│  │                                                                               │ │
│  │  对比模式选择：                                                                │ │
│  │  [双窗口对照] [滑块对比] [差值图像] [伪彩色对比]                                │ │
│  │                                                                               │ │
│  │  差值图像说明：                                                                │ │
│  │  显示 Output - Input 的差值，高亮显示 CCM 带来的色彩漂移                        │ │
│  │  或去马赛克产生的锯齿                                                          │ │
│  │                                                                               │ │
│  └───────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                     │
...
| 模式 | 功能 | 用途 |
|------|------|------|
| 双窗口对照 | 左右并排显示输入和输出 | 快速对比整体效果 |
| 滑块对比 | 拖拽滑块实时切换显示 | 精细对比局部区域 |
| 差值图像 | 显示 Output - Input 的差值 | 定位色彩漂移或锯齿 |
| 伪彩色对比 | 用伪彩色标记差异区域 | 高亮显示变化区域 |

#### 33.2.2 直方图与信息增强

**建议**：在图像窗口侧边集成实时高位深直方图（支持 16-bit 线性显示）。

**优化点**：必须提供"伪彩色映射（False Color Mapping）"功能。对于超高动态范围图像，直观地用伪彩色标记出哪些像素处于"过曝"区（Max_DN）或"死黑"区（0 DN）。
...
│                                                                                     │
│  ┌───────────────────────────────────────────────────────────────────────────────┐ │
│  │  Histogram & False Color Mapping                                               │ │
│  ├───────────────────────────────────────────────────────────────────────────────┤ │
│  │                                                                               │ │
│  │  ┌─────────────────────────────────┬─────────────────────────────────┐        │ │
│  │  │                                 │                                 │        │ │
│  │  │    图像显示                      │    直方图显示                    │        │ │
│  │  │    (伪彩色映射模式)              │    (16-bit 线性直方图)           │        │ │
│  │  │                                 │                                 │        │ │
│  │  │    ┌───────────────────────┐    │    ┌───────────────────────┐    │        │ │
│  │  │    │                       │    │    │                       │    │        │ │
│  │  │    │  🔴 过曝区域 (Max_DN) │    │    │   ████████████████████ │    │        │ │
...
│  │  │  像素值范围        │ 伪彩色   │ 说明                                      │ │ │
│  │  ├─────────────────────────────────────────────────────────────────────────┤ │ │
│  │  │  [0, 10]          │ 🔵 蓝色  │ 死黑区域（可能为 OB 扣除过度）              │ │ │
│  │  │  [10, Max_DN*0.1] │ 🟡 黄色  │ 暗部区域（信噪比较低）                      │ │ │
│  │  │  [Max_DN*0.1, Max_DN*0.9] │ 🟢 绿色 │ 正常区域（动态范围充分利用）          │ │ │
│  │  │  [Max_DN*0.9, Max_DN] │ 🔴 红色 │ 高光区域（接近饱和）                      │ │ │
│  │  │  > Max_DN         │ ⚪ 白色  │ 过曝区域（ADC 溢出）                        │ │ │
│  │  └─────────────────────────────────────────────────────────────────────────┘ │ │
│  │                                                                               │ │
│  │  显示模式选择：                                                                │ │
│  │  [原始图像] [伪彩色映射] [线性映射] [Gamma 预览]                                │ │
│  │                                                                               │ │
│  └───────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                     │
└─────────────────────────────────────────────────────────────────────────────────────┘
...
#### 33.3.1 参数来源标识 (Source Tagging)

**建议**：如文档中所述，给参数打上 `[SCM]`、`[Manual]` 等标签。

**视觉提醒**：当用户在 UI 上手动修改了原本由 SCM 自动加载的参数时，标签应自动变更为 `[Manual-Override]`（高亮色显示），并提供一个明显的"重置"按钮，点击即可恢复标定原始值。
...
│  │  B Gain:                                                                      │ │
│  │  ┌────────────────────────────────────────────┬───────────────────────────┐  │ │
│  │  │ [0.85 🟡]                                    │ [Manual-Override ▼]       │  │ │
│  │  └────────────────────────────────────────────┴───────────────────────────┘  │ │
│  │  来源：🟡 手动覆盖（原值 0.80）                                                 │ │
│  │                                                                               │ │
│  │  ┌─────────────────────────────────────────────────────────────────────┐     │ │
│  │  │  ⚠️ 提示：B Gain 已被手动修改，偏离标定值                              │     │ │
│  │  │  [重置为标定值] 恢复 Quick Calib 的原始值 (0.80)                      │     │ │
│  │  └─────────────────────────────────────────────────────────────────────┘     │ │
│  │                                                                               │ │
│  └───────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                     │
...
| 标签 | 颜色 | 说明 | 点击行为 |
|------|------|------|----------|
| `[SCM]` | 绿色 | 参数来自 SCM 测试 | 下拉菜单可切换来源 |
| `[Color Char]` | 蓝色 | 参数来自 Color Char 测试 | 下拉菜单可切换来源 |
| `[Quick Calib]` | 紫色 | 参数来自 Quick Calib 测试（当前使用） | 下拉菜单可切换来源 |
| `[Manual]` | 灰色 | 用户手动输入 | 无下拉菜单 |
| `[Manual-Override] 🟡` | 黄色（高亮） | 用户手动修改了标定参数 | 显示"重置为标定值"按钮 |

#### 33.3.2 一键防呆预检查

**建议**：在执行"计算/标定"操作的按钮旁，增加一个"配置预检查"图标。

**逻辑**：点击后 UI 自动高亮提示当前缺失的依赖，而不是等报错后再弹窗提醒。
...
│  │  │  ⚪ N2 (LSC)     已禁用                                             │     │ │
│  │  │  ✅ N3 (Denoise) 参数来源: SCM                                      │     │ │
│  │  │  ✅ N4 (Demosaic) 算法: Edge-Aware                                  │     │ │
│  │  │  ✅ N5 (WB)      参数来源: Quick Calib                              │     │ │
│  │  │  ⚠️ N6 (CCM)     ⚠️ 缺少依赖：需要先加载 Color Char 或 Quick Calib   │     │ │
│  │  │  ✅ N7 (TM)      输入范围: [0, 4095]                                 │     │ │
│  │  │  ✅ N8 (Gamma)   Gamma 值: 2.2                                      │     │ │
│  │  │                                                                     │     │ │
│  │  │  ────────────────────────────────────────────────────────────────── │     │ │
│  │  │                                                                     │     │ │
│  │  │  ⚠️ 警告：                                                          │     │ │
│  │  │                                                                     │     │ │
│  │  │  1. N6 (CCM) 节点缺少标定数据                                        │     │ │
│  │  │     原因：Color Char 和 Quick Calib 数据均未加载                     │     │ │
│  │  │     影响：色彩校正将使用默认单位矩阵（无色彩校正）                      │     │ │
│  │  │     解决：[点击此处加载 Color Char 数据]                              │     │ │
│  │  │                                                                     │     │ │
│  │  │  2. N2 (LSC) 节点已禁用                                              │     │ │
│  │  │     影响：镜头阴影校正未生效，边缘可能出现偏暗                        │     │ │
│  │  │     解决：[启用 LSC 节点] 或 [加载 Gain Map]                          │     │ │
│  │  │                                                                     │     │ │
│  │  │  ────────────────────────────────────────────────────────────────── │     │ │
│  │  │                                                                     │     │ │
│  │  │  [✓ 已解决所有问题，开始运行]                                        │     │ │
...
| 节点 | 必要依赖 | 缺失时的 UI 提示 |
|------|----------|-----------------|
| N0 (OB) | SCM | 🔴 需要先运行 SCM 测试 |
| N1 (DPC) | SCM | 🔴 需要先运行 SCM 测试 |
| N2 (LSC) | Gain Map 文件 | 🟡 Gain Map 未加载（可禁用） |
| N3 (Denoise) | SCM (可选) | 🟢 SCM 数据建议加载 |
| N4 (Demosaic) | 无 | 🟢 算法默认为 Edge-Aware |
| N5 (WB) | Color Char 或 Quick Calib | 🔴 需要加载标定数据 |
| N6 (CCM) | Color Char 或 Quick Calib | 🔴 需要加载标定数据 |
| N7 (TM) | SCM (Max_DN) | 🟡 SCM 数据建议加载 |
| N8 (Gamma) | Quick Calib (可选) | 🟢 默认 Gamma = 2.2 |

### 33.4 辅助理解的视觉化建议

为了帮助用户理解 Pipeline 逻辑以及数据在线性/非线性空间中的变化，建议在 UI 的帮助页面或状态面板中增加以下视觉图示：
...
│                                                                                     │
│  ┌───────────────────────────────────────────────────────────────────────────────┐ │
│  │  Pipeline Data Space Visualization                                             │ │
│  ├───────────────────────────────────────────────────────────────────────────────┤ │
│  │                                                                               │ │
│  │  线性空间 vs 非线性空间流转：                                                   │ │
│  │                                                                               │ │
...
│  │  │  【纯线性物理空间】                                                      │   │ │
│  │  │  ┌─────────────────────────────────────────────────────────────┐       │   │ │
│  │  │  │  N0(OB) ── N1(DPC) ── N2(LSC) ── N3(DN) ── N4(DM)            │       │   │ │
│  │  │  │    🟢       🟢        🟢        🟢       🟢                   │       │   │ │
│  │  │  │    │         │         │         │        │                   │       │   │ │
│  │  │  │    └─────────┴─────────┴─────────┴────────┘                   │       │   │ │
│  │  │  └─────────────────────────────────────────────────────────────┘       │   │ │
...
│  │  │                                                                         │   │ │
│  │  │  重要规则：                                                              │   │ │
│  │  │  1. WB 和 CCM 必须在 Gamma 之前执行（线性空间）                           │   │ │
│  │  │  2. 矩阵运算（如 CCM）只能在线性空间进行                                  │   │ │
│  │  │  3. Gamma 后的数据不可再进行线性运算                                      │   │ │
│  │  │                                                                         │   │ │
│  │  └───────────────────────────────────────────────────────────────────────┘   │ │
│  │                                                                               │ │
│  └───────────────────────────────────────────────────────────────────────────────┘ │
...
确保 UI 层（wxUI）只负责参数显示和交互，真正的 ImagePipeline 逻辑应封装在独立的 C++ 核心库中。
...
│  │  ┌───────────────────────────────┐  ┌───────────────────────────────┐        │ │
│  │  │                               │  │                               │        │ │
│  │  │   UI 层 (wxWidgets/wxAui)      │  │   核心逻辑层 (Core Library)    │        │ │
│  │  │                               │  │                               │        │ │
│  │  │   ├─ PipelinePanel            │  │   ├─ ImagePipeline            │        │ │
│  │  │   ├─ NodeParameterPanel       │  │   ├─ ImageNode (Base)         │        │ │
│  │  │   ├─ ImageDisplayPanel        │  │   ├─ OBNode                   │        │ │
│  │  │   ├─ HistogramPanel           │  │   ├─ DPCNode                  │        │ │
│  │  │   ├─ CalibrationStatusPanel   │  │   ├─ LSCNode                  │        │ │
│  │  │   └─ ConsoleLogPanel          │  │   ├─ DenoiseNode              │        │ │
│  │  │                               │  │   ├─ DemosaicNode             │        │ │
│  │  │                               │  │   ├─ WBNode                   │        │ │
│  │  │                               │  │   ├─ CCMNode                  │        │ │
│  │  │                               │  │   ├─ ToneMappingNode          │        │ │
...
│  │  │                               │  │                               │        │ │
│  │  │   交互接口：                   │  │   数据接口：                   │        │ │
│  │  │   ├─ GetParameter()           │──▶ ├─ GetWBGains()              │        │ │
│  │  │   ├─ SetParameter()           │◀──├─ SetWBGains()              │        │ │
│  │  │   ├─ RunPipeline()            │──▶ ├─ Process()                 │        │ │
│  │  │   ├─ GetStatus()              │◀──├─ GetNodeStatus()            │        │ │
│  │  │   └─ GetImage()               │◀──├─ GetOutputImage()           │        │ │
│  │  │                               │  │                               │        │ │
│  │  └───────────────────────────────┘  └───────────────────────────────┘        │ │
│  │                                                                               │ │
│  │  ─────────────────────────────────────────────────────────────────────────── │ │
│  │                                                                               │ │
│  │  优势：                                                                       │ │
│  │  1. UI 框架可替换：将来换成 Qt 或 Web 前端，核心算法无需重写                     │ │
│  │  2. 核心逻辑独立测试：可以脱离 UI 进行单元测试和集成测试                         │ │
│  │  3. 多平台支持：核心库可跨平台使用（Windows/Linux/MacOS）                       │ │
│  │  4. 性能优化：核心库可独立进行 SIMD 优化，不影响 UI                             │ │
│  │                                                                               │ │
│  └───────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                     │
└─────────────────────────────────────────────────────────────────────────────────────┘
...
public:
    // 获取节点参数
    virtual double GetParameter(int node_id, const std::string& param_name) = 0;
    
    // 设置节点参数
    virtual void SetParameter(int node_id, const std::string& param_name, double value) = 0;
    
    // 获取参数来源
    virtual std::string GetParameterSource(int node_id, const std::string& param_name) = 0;
    
    // 运行 Pipeline
    virtual cv::Mat RunPipeline(const cv::Mat& input) = 0;
    
    // 获取节点状态
    virtual NodeStatus GetNodeStatus(int node_id) = 0;
    
    // 加载标定数据
    virtual bool LoadCalibrationData(const std::string& source, const std::string& path) = 0;
    
    // 获取输出图像
    virtual cv::Mat GetOutputImage() = 0;
    
    // 获取中间图像（Debug 模式）
    virtual cv::Mat GetIntermediateImage(int node_id) = 0;
};
```

#### 33.5.2 日志系统

在 UI 底部保留一个"控制台输出窗口"，将 Pipeline 中的每一行警告实时打在 UI 上，这比查看日志文件高效得多。
...
│  │  ─────────────────────────────────────────────────────────────────────────── │ │
│  │                                                                               │ │
│  │  14:32:15  INFO    Pipeline    Pipeline initialized with 9 nodes             │ │
│  │  14:32:16  INFO    SCM         Loading SCM results from scm_results.toml     │ │
│  │  14:32:17  INFO    SCM         dark_offset_dn = 64, dark_noise_dn = 30       │ │
│  │  14:32:18  INFO    QC          Loading Quick Calib results                   │ │
│  │  14:32:19  INFO    QC          WB gains: R=1.25, G=1.00, B=0.80              │ │
│  │  14:32:20  WARN    CCM         ⚠️ CCM row 0 sum (0.85) deviates from 1.0      │ │
│  │  14:32:21  INFO    Pipeline    Running Pipeline with input image (640x480)   │ │
│  │  14:32:22  INFO    N0          OB processing: offset = 64 DN                 │ │
│  │  14:32:23  INFO    N1          DPC processing: 5 bad pixels detected         │ │
│  │  14:32:24  INFO    N4          Demosaic: using Edge-Aware algorithm          │ │
│  │  14:32:25  WARN    TM          ⚠️ 0.5% pixels exceed Max_DN (4095)            │ │
│  │  14:32:26  INFO    Pipeline    Pipeline completed in 45.2 ms (22.1 FPS)      │ │
│  │                                                                               │ │
│  │  ─────────────────────────────────────────────────────────────────────────── │ │
│  │                                                                               │ │
│  │  ┌─────────────────────────────────────────────────────────────────────┐     │ │
│  │  │  [Clear] [Export Log] [Filter: All ▼]                                │     │ │
│  │  └─────────────────────────────────────────────────────────────────────┘     │ │
│  │                                                                               │ │
│  │  日志级别颜色：                                                                │ │
│  │  INFO  = 白色                                                                 │ │
...
```cpp
class ConsoleLogPanel : public wxPanel {
public:
    // 添加日志消息
    void AddLogMessage(LogLevel level, const std::string& source, const std::string& message);
    ...
    // 设置日志级别过滤
    void SetLogLevelFilter(LogLevel min_level);
...
// 使用示例
void PipelinePanel::OnPipelineWarning(const std::string& message) {
    console_log_->AddLogMessage(LogLevel::WARN, "CCM", message);
    // UI 上实时显示警告，比查看日志文件高效得多
}
```